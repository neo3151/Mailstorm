...
OUTPUT_FILE = r"C:\Users\neo31\Mailstorm\releases\Mailstorm_Ep7_Abyssal_Edition.pdf"
...
COVER = get_latest(ARCHIVE_LOCAL, "ep7_cover_hazard_orange")
STORY_PANELS = [
    get_latest(ARCHIVE_LOCAL, "ep7_01_incubation"),
    get_latest(ARCHIVE_LOCAL, "ep7_02_host_chosen_safe"),
    get_latest(ARCHIVE_LOCAL, "ep7_03_guest_star"),
    get_latest(ARCHIVE_LOCAL, "ep7_04_metric_assault"),
    get_latest(ARCHIVE_LOCAL, "ep7_05_aftermath"),
]

DOSSIER_CHAIRMAN = get_latest(ARCHIVE_LOCAL, "ep7_dossier_chairman")
...
CAPTIONS = {
    0:  ("TL", "THE DEEP LOADING DOCK OF THE ZIGGURAT. IT PROMISES THE CLIPBOARD."),
    1:  ("BR", "THE 204B PARASITE. IT DIGESTS SOLIDARITY. PURE METRICS ONLY."),
    2:  ("TL", "THE CHAIRMAN TAKES HIS SEAT. GRIEVANCE MANA AT MAXIMUM DENSITY."),
    3:  ("BR", "THE METRIC ASSAULT FAILS. REDPLUM LASERS DEFLECTED BY THE MITTENS."),
    4:  ("TL", "THE 1% CANNOT BREAK THE STEWARD. THE FILIBUSTER HOLDS."),
}
...
def assemble():
    ...
    pdf.set_title("Mailstorm: Episode 7 - The 204b Parasite (Abyssal Edition)")
    pdf.set_author("The Department of Logistics")
    total, done = 11, 0  # approx pages
    def prog(label):
        nonlocal done; done += 1
        filled = int(20*done/total)
        print(f"  [{'█'*filled}{'░'*(20-filled)}] {int(100*done/total):3d}% | {done}/{total} | {label}")

    print("--- ASSEMBLING EPISODE 7 ABYSSAL EDITION ---")
    add_img(pdf, COVER); prog("Cover Image")
    black_page(pdf, "PURE METRICS ONLY."); prog("Inside Front")
    ...
    black_page(pdf, "THE ALGORITHM NEVER FORGETS."); prog("Epilogue Blank")
    
    # Append generic/lore artifacts at the end
    add_img(pdf, DOSSIER_CHAIRMAN); prog("Dossier: The Chairman")
